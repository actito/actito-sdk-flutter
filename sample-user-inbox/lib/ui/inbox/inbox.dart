...
import 'package:actito_user_inbox/actito_user_inbox.dart';
...
class _InboxViewState extends State<InboxView> {
  StreamSubscription<ActitoNotification>? _onNotificationOpenedSubscription;
  StreamSubscription<ActitoNotificationReceivedEvent>?
      _onNotificationReceivedSubscription;

  List<ActitoUserInboxItem> _items = [];
  ...
  _setupListeners() {
    ...
    _onNotificationOpenedSubscription =
        ActitoPush.onNotificationOpened.listen((event) {
      _refresh();
    });
  }
  ...
  void _showModalBottomSheet(ActitoUserInboxItem item) {
  ...
  void _onOpenClicked(ActitoUserInboxItem item) async {
    try {
  ...
  void _onMarkAsReadClicked(ActitoUserInboxItem item) async {
    try {
  ...
  void _onRemoveClicked(ActitoUserInboxItem item) async {
    try {
  ...
  void _refresh() async {
    try {
      ...
      final userInboxResponse =
          await ActitoUserInbox.parseResponseFromString(requestResponse);
